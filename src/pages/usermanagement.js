...
const Usermanagement = (showSidebar) => {
  ...
  const handleCheckboxChange = (index) => {
    ...
    if (updatedUsers[index]) {
      if (typeof updatedUsers[index].checked === "undefined") {
        updatedUsers[index].checked = false;
      }
      ...
      const selectedIds = updatedUsers
        .filter((user) => user.checked)
        .map((user) => user._id);
      setSelectedUserIds(selectedIds);
    } else {
      console.error(
        `User at index ${index} is undefined. Users array:`,
        updatedUsers
      );
  ...
  const handleAddUser = () => {
    ...
    axios
      .post("http://localhost:5000/users/add-user", newUser)
      .then((response) => {
  ...
  const handleSearch = () => {
    ...
    const { newUserIdentifier, newUserGroup, newUserStatus, createdOn } =
      searchParams;

    // Construct the search query based on the selected filters
    const queryParams = {newUserIdentifier,newUserGroup,newUserStatus,createdOn,};
    console.log("Sending request with newUserStatus:", newUserStatus);

    // Make a request to the server to fetch the filtered users
    axios
      .get("http://localhost:5000/users/search", { params: queryParams })
      .then((response) => {
  ...
  const handleDeleteUsers = () => {
    ...
    // Make a request to the server to delete the selected users
    axios
      ...
      .then((response) => {
        ...
        // Update the local state to reflect the deleted users
      ...
      .catch((error) => {
        ...
        alert("Error deleting users. Please try again later.");
      });
  };

  return (
    <div
    key={`${key}-${showAddUserScreen}`}
    ...
    >
      <div class="container">
        <div style={{ display: "flex", alignItems: "center" }}>
          ...
          <button
            className="userbtn"
            onClick={() => setShowAddUserScreen(true)}
          >
        ...
        <div class="table-responsive custom-table-responsive">
          <div
            style={{border: "2px solid #ccc",paddingTop: "30px",paddingBottom: "30px",background: "#ffffff",borderRadius: "15px",}}
            className="resp1"
          >
            <div
              style={{display: "flex",marginLeft: "20px",flexDirection: "row",flexWrap: "wrap",}}
            >
              <div
                style={{position: "relative",border: "1px solid #ccc",borderRadius: "8px",marginRight: "10px",marginBottom: "10px",width: "300px",height: "40px",}}
              >
                <input
                  type="text"
                  placeholder="Search.."
                  style={{position: "absolute",backgroundColor: "transparent",border: "none",  outline: "none",padding: "10px 25px",fontSize: 17,
                  }}
              ...
              <div style={{position: "relative",marginRight: "10px",marginBottom: "10px",}}className="userstate">
                ...
                <div  style={{border: "1px solid #ccc",borderRadius: "8px",width: "200px",height: "40px",}}>
                  <select
                    style={{  backgroundColor: "transparent",  border: "none",    outline: "none",  padding: "5px 10px",  fontSize: 16,  width: "100%",height: "100%",}}
                    value={searchParams.newUserGroup}
                    onChange={(e) =>
                      setSearchParams({
                        ...searchParams,
                        newUserGroup: e.target.value,
                      })
                    }
                  >
              ...
              <div style={{  position: "relative",marginRight: "10px",marginBottom: "10px",}}
                className="date">
                <div
                  style={{position: "absolute",  top: "-8px",  background: "#fff",  paddingLeft: "19px",  color: "rgb(117 117 117)",width: "104px",  height: "10px",    }}
                >
                  Creation Date
                </div>
                <div style={{    border: "1px solid #ccc",  borderRadius: "8px",  width: "172px",  height: "40px",  }}>
                  <input
                    type="date"
                    style={{  backgroundColor: "transparent",  border: "none",outline: "none",padding: "5px 10px",  fontSize: 16,height: "100%",}}
            ...
            <div  style={{    display: "flex",flexDirection: "row",flexWrap: "wrap",  marginTop: "40px",  position: "relative",marginLeft: "20px",}}  >
              ...
              <div  className=" iconresponsive"
                style={{backgroundColor: "#e7e9ef",  marginLeft: "13px",  opacity: "0.7",  borderRadius: "5px",  display: "flex",  alignItems: "center",  justifyContent: "center",    width: "45px",  height: "42px",  }}>
                <MdDelete
              ...
              <div className=" iconresponsive" style={{ display: "flex" }}>
                  <p  style={{color: "grey",marginLeft: "10px",marginTop: "10px",textDecoration: "underline",cursor: "pointer",}}
                    ...
                    <tr key={index} style={{ width: "100%" }}>
                      ...
                      <td>
                        {editableUserIndex === index ? (
                          <input style={{outline: "none",    borderRadius: "5px",  height: "11px",  padding: "12px",width: "150px",marginLeft: "5px",}}
                            type="text"
                      ...
                      <td>
                        {editableUserIndex === index ? (
                          <input
                            ...
                            onChange={(e) => {
                              const updatedUsers = [...users];
                              updatedUsers[index].newUserUsername =
                                e.target.value;
                              setUsers(updatedUsers);
                            }}
                          />
                      ...
                      <td>
                        {editableUserIndex === index ? (
                          <input style={{  outline: "none",borderRadius: "5px",  height: "11px",      padding: "12px",  width: "150px",}}
                            type="text"
                          ...
                          <p style={{ color: "black" }}>{user.newUserEmail}</p>
                        )}
                      </td>
                      ...
                      <td>
                        {editableUserIndex === index ? (
                          <select
                            value={user.newUserStatus}
                            onChange={(e) => {
                              const updatedUsers = [...users];
                              updatedUsers[index].newUserStatus =
                                e.target.value;
                              setUsers(updatedUsers);
                              handleStatusChange(index, e);
                            }}
      ...
      {showAddUserScreen && (
        <div
          style={{position: "fixed",top: 0,  left: 0,  width: "100%",height: "100%",background: "rgba(0, 0, 0, 0.5)",display: "flex",justifyContent: "center",
            alignItems: "center",
          }}
        >
          <div
            style={{width: "500px",  height: "600px",  background: "rgb(248, 250, 251)", borderRadius: "10px",display: "flex",flexDirection: "column",alignItems: "center",
            }}
          >
            <header
              className="fw-bold"
              style={{backgroundColor: "#050e2d",  fontSize: "20px",  color: "white",  fontWeight: "bold",  paddingLeft: "10px",  borderRadius: "10px 10px 0 0",  width: "490px",  height: "60px",
              }}
            >
              Add New User
              <IoMdClose
                style={{ marginLeft: "300px" }}
                onClick={() => setShowAddUserScreen(false)}
              />
            </header>
            <div style={{ marginRight: "14px" }}>
              <div  style={{  display: "flex",      flexDirection: "column",  width: "80%",marginTop: "20px",}}>
                <label htmlFor="name" style={{  color: "#050e2d",padding: "3px",fontWeight: "bold",}}>
                  Full Name
                </label>
                <input
                  type="text"
                  id="name"
                  style={{outline: "none",  borderColor: "oldlace",  borderRadius: "5px",  height: "11px",  padding: "12px",  width: "400px",}}
                  value={newUserName}
              ...
              <div  style={{  display: "flex",  flexDirection: "column",  width: "80%",marginTop: "20px",}}>
                <label  htmlFor="username"  style={{  color: "#050e2d",  padding: "3px",  fontWeight: "bold",  }}>
                  User Name
                </label>
                <input
                  type="text"
              ...
              <div
                style={{  display: "flex",  flexDirection: "column",  width: "80%",  marginTop: "20px",}}
              >
                <label
                  htmlFor="email"
                  style={{  color: "#050e2d",    padding: "3px",fontWeight: "bold",  }}
                >
                  Email Address
                </label>
                <input
              ...
              >
                <p
                  style={{  color: "grey",textDecoration: "underline",cursor: "pointer",margintop: "7px",  marginRight: "auto",
                  }}
                  onClick={resetForm}
                >
                  Reset Fields
                ...
                <div>
                  <button className="userbtn2" onClick={handleAddUser}>
                    Add User
                  </button>
                </div>
              </div>
